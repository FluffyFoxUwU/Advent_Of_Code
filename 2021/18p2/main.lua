...
for line in io.lines() do
  snailNumbers[#snailNumbers + 1] = line
...
if false then
local resJSON = add(JSON.decode("[[2,[[7,7],7]],[[5,8],[[9,3],[0,2]]]]"),
                JSON.decode("[[[0,[5,8]],[[1,7],[9,6]]],[[4,[1,2]],[[1,4],2]]]"))
local res = JSON.decode(resJSON)

print(resJSON)
print(magnitude(res[1], res[2]))
...
local largest = 0
for _, op1 in ipairs(snailNumbers) do
  for _, op2 in ipairs(snailNumbers) do
    local result = JSON.decode(add(JSON.decode(op1), JSON.decode(op2)))
    local mag = magnitude(result[1], result[2])
    if mag > largest then
      print(mag)
      largest = mag
    end 
